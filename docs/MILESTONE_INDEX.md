...
## HV25 HGATP Candidate Payload and Stage2 Activation Plan Foundation

- **Purpose:** Build executable software-only HGATP candidate payload and stage2 activation-plan foundations before any future guarded `hgatp` write path.
- **Actual capability proven:** The kernel derives an HGATP candidate from VM/vCPU, guest memory, guest address-space, second-stage metadata, stage2 software table metadata, H-extension discovery state, and CSR safety state. It validates mode, VMID, root PPN alignment, write-attempt, active-stage2, checksum, lifecycle, mutation, and corruption invariants. The stage2 activation plan consumes the validated HGATP candidate plus stage2 metadata, table metadata, H-extension state, and CSR safety state to derive blockers, next action, checksums, readiness, and counters.
- **Key commands added:** `hv hgatp`, `hv-hgatp`, `hv hgatp build`, `hv hgatp validate`, `hv hgatp blockers`, `hv hgatp fields`, `hv hgatp candidate`, `hv hgatp checksum`, `hv hgatp reset`, `hv stage2-plan`, `hv-stage2-plan`, `hv stage2-plan build`, `hv stage2-plan validate`, `hv stage2-plan blockers`, `hv stage2-plan next`, `hv stage2-plan checksum`, `hv stage2-plan reset`, and HV25 invariant/negative-test commands.
