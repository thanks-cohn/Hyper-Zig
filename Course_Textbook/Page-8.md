...
In HV07, the hypervisor learned how a guest views memory.

Now we encounter a new question.

Suppose a guest is running.
...
* the guest performs an illegal operation
* the hypervisor needs control back
...
A Guest Exit is one of the most important concepts in virtualization.

A hypervisor spends much of its life doing two things:
...
```text
Handle the reason it stopped.
```

HV08 focuses on the second part.

At this stage we are not executing a guest.

We are not trapping real instructions.

We are not handling real virtualization events.

Instead, we are building the structure that will eventually record:

* why the guest stopped
* where it stopped
* what instruction was involved
* what VM was running
* what vCPU was running

Think of Guest Exit Metadata as an incident report.

Something happened.

The hypervisor needs evidence.

This module stores that evidence.
...
Guest Exit Metadata answers:

```text
Why did execution stop?
```

Both are required for virtualization.
...
```text
kernel/hypervisor/guest_exit.zig
```
...
1. Exit records belong to a VM.

2. Exit records belong to a vCPU.

3. Exit reasons are recorded.

4. Program counter information is recorded.

5. Trap information is recorded.

6. Ownership information is recorded.

7. Exit state is observable.

8. Exit metadata does not handle exits.

9. Exit metadata records exits.

10. Recording and handling are different concepts.
...
In HV06, you created Guest Entry.

Now create Guest Exit Metadata.

The goal is not to trap real instructions.

The goal is to model how a hypervisor records information about a guest exit.
...
static GuestExitRecord guest_exit;

void guest_exit_init(
    VmId vm_id,
    VcpuId vcpu_id)
{
    /*
...
Why should a hypervisor record why a guest stopped?

2.

Why record the program counter?

3.

Why should exit records belong to a VM?

4.

Why should exit records belong to a vCPU?
...
Why should exit information be observable?

### Challenge Question

Suppose the hypervisor only records:

```text
Guest stopped.
```

Would that be enough information to debug a failure?
...
Before moving to HV09, make sure you can explain:

* what a Guest Exit is
* why exit metadata exists
* why ownership is recorded
* why trap information matters
* why recording and handling are separate concepts
...
Students should understand that virtualization is not only about starting execution.

It is also about regaining control safely.

A hypervisor must know:

```text
Why did the guest stop?
```
...
This module introduces the concept of evidence collection.
...
Many guest exits are expected and normal.
...
```text
Recording an event fixes the event.
```
...
```text
A guest exit only needs a reason.
```

Correction:

The hypervisor often needs:
...
what information would you want before fixing it?
```

Relate that answer to Guest Exit Metadata.
...
* what a Guest Exit is
* why exit metadata exists
* why exit reasons matter
* why ownership is recorded
* why evidence collection matters

### Connection To Future Modules

HV06 prepared execution.

HV08 prepares failure observation.
...
The hypervisor now understands both ends of execution.

Future modules will begin using this information to control guest run attempts.

### Key Idea

A good hypervisor does not merely start guests.

A good hypervisor understands why they stop.
