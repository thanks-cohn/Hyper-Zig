...
fn handle(cmd: []const u8) void {
    ...
    if (equals(cmd, "hv hgatp") or equals(cmd, "hv-hgatp") or equals(cmd, "hv hgatp status")) return hv.printHgatp();
    if (equals(cmd, "hv hgatp build")) return hv.buildHgatp();
    if (equals(cmd, "hv hgatp validate")) return hv.validateHgatp();
    if (equals(cmd, "hv hgatp blockers")) return hv.blockersHgatp();
    if (equals(cmd, "hv hgatp fields")) return hv.fieldsHgatp();
    if (equals(cmd, "hv hgatp candidate")) return hv.candidateHgatp();
    if (equals(cmd, "hv hgatp checksum")) return hv.checksumHgatp();
    if (equals(cmd, "hv hgatp reset")) return hv.resetHgatp();
    if (equals(cmd, "hv hgatp invariant-lifecycle-test")) return hv.hgatpLifecycleInvariant();
    if (equals(cmd, "hv hgatp invariant-derivation-test")) return hv.hgatpDerivationInvariant();
    if (equals(cmd, "hv hgatp invariant-corruption-test")) return hv.hgatpCorruptionInvariant();
    if (equals(cmd, "hv hgatp require-hext-test")) return hv.hgatpRequireHextTest();
    if (equals(cmd, "hv hgatp mode-test")) return hv.hgatpModeTest();
    if (equals(cmd, "hv hgatp ppn-alignment-test")) return hv.hgatpPpnAlignmentTest();
    if (equals(cmd, "hv hgatp vmid-bounds-test")) return hv.hgatpVmidBoundsTest();
    if (equals(cmd, "hv hgatp write-attempt-test")) return hv.hgatpWriteAttemptTest();
    if (equals(cmd, "hv hgatp active-stage2-test")) return hv.hgatpActiveStage2Test();
    if (equals(cmd, "hv stage2-plan") or equals(cmd, "hv-stage2-plan") or equals(cmd, "hv stage2-plan status")) return hv.printStage2Plan();
    if (equals(cmd, "hv stage2-plan build")) return hv.buildStage2Plan();
    if (equals(cmd, "hv stage2-plan validate")) return hv.validateStage2Plan();
    if (equals(cmd, "hv stage2-plan blockers")) return hv.blockersStage2Plan();
    if (equals(cmd, "hv stage2-plan next")) return hv.nextStage2Plan();
    if (equals(cmd, "hv stage2-plan checksum")) return hv.checksumStage2Plan();
    if (equals(cmd, "hv stage2-plan reset")) return hv.resetStage2Plan();
    if (equals(cmd, "hv stage2-plan invariant-lifecycle-test")) return hv.stage2PlanLifecycleInvariant();
    if (equals(cmd, "hv stage2-plan invariant-consumption-test")) return hv.stage2PlanConsumptionInvariant();
    if (equals(cmd, "hv stage2-plan invariant-corruption-test")) return hv.stage2PlanCorruptionInvariant();
    if (equals(cmd, "hv stage2-plan require-hgatp-test")) return hv.stage2PlanRequireHgatpTest();
    if (equals(cmd, "hv stage2-plan require-stage2-test")) return hv.stage2PlanRequireStage2Test();
    if (equals(cmd, "hv stage2-plan require-table-test")) return hv.stage2PlanRequireTableTest();
    if (equals(cmd, "hv stage2-plan require-csr-safety-test")) return hv.stage2PlanRequireCsrSafetyTest();
    if (equals(cmd, "hv stage2-plan active-stage2-test")) return hv.stage2PlanActiveStage2Test();
    if (equals(cmd, "hv stage2-plan write-attempt-test")) return hv.stage2PlanWriteAttemptTest();
    if (equals(cmd, "hv hv25 invariant-all")) return hv.hv25InvariantAll();
    if (equals(cmd, "hv guest-entry") or equals(cmd, "hv-guest-entry") or equals(cmd, "hv guest-entry status")) return hv.printGuestEntryFrame();
    if (equals(cmd, "hv guest-entry build")) return hv.buildGuestEntryFrame();
    if (equals(cmd, "hv guest-entry validate")) return hv.validateGuestEntryFrame();
    if (equals(cmd, "hv guest-entry blockers")) return hv.blockersGuestEntryFrame();
    if (equals(cmd, "hv guest-entry fields")) return hv.fieldsGuestEntryFrame();
    if (equals(cmd, "hv guest-entry registers")) return hv.registersGuestEntryFrame();
    if (equals(cmd, "hv guest-entry checksum")) return hv.checksumGuestEntryFrame();
    if (equals(cmd, "hv guest-entry reset")) return hv.resetGuestEntryFrame();
    if (equals(cmd, "hv guest-entry invariant-lifecycle-test")) return hv.gefLifecycle();
    if (equals(cmd, "hv guest-entry invariant-mutation-test")) return hv.gefMutation();
    if (equals(cmd, "hv guest-entry require-activation-plan-test")) return hv.gefRequireActivation();
    if (equals(cmd, "hv guest-entry require-context-test")) return hv.gefRequireContext();
    if (equals(cmd, "hv guest-entry pc-mutation-test")) return hv.gefPcMutation();
    if (equals(cmd, "hv guest-entry sp-mutation-test")) return hv.gefSpMutation();
    if (equals(cmd, "hv guest-entry register-corruption-test")) return hv.gefRegisterCorruption();
    if (equals(cmd, "hv guest-entry guest-entered-test")) return hv.gefGuestEntered();
    if (equals(cmd, "hv guest-entry instruction-executed-test")) return hv.gefInstruction();
    if (equals(cmd, "hv trap-return") or equals(cmd, "hv-trap-return") or equals(cmd, "hv trap-return status")) return hv.printTrapReturnFrame();
    if (equals(cmd, "hv trap-return build")) return hv.buildTrapReturnFrame();
    if (equals(cmd, "hv trap-return validate")) return hv.validateTrapReturnFrame();
    if (equals(cmd, "hv trap-return blockers")) return hv.blockersTrapReturnFrame();
    if (equals(cmd, "hv trap-return fields")) return hv.fieldsTrapReturnFrame();
    if (equals(cmd, "hv trap-return checksum")) return hv.checksumTrapReturnFrame();
    if (equals(cmd, "hv trap-return reset")) return hv.resetTrapReturnFrame();
    if (equals(cmd, "hv trap-return invariant-lifecycle-test")) return hv.trfLifecycle();
    if (equals(cmd, "hv trap-return invariant-mutation-test")) return hv.trfMutation();
    if (equals(cmd, "hv trap-return require-entry-frame-test")) return hv.trfRequireEntry();
    if (equals(cmd, "hv trap-return require-csr-safety-test")) return hv.trfRequireCsr();
    if (equals(cmd, "hv trap-return require-hext-test")) return hv.trfRequireHext();
    if (equals(cmd, "hv trap-return sepc-corruption-test")) return hv.trfSepc();
    if (equals(cmd, "hv trap-return status-corruption-test")) return hv.trfStatus();
    if (equals(cmd, "hv trap-return executed-test")) return hv.trfExecuted();
    if (equals(cmd, "hv first-instruction") or equals(cmd, "hv-first-instruction") or equals(cmd, "hv first-instruction status")) return hv.printFirstInstructionPlan();
    if (equals(cmd, "hv first-instruction build")) return hv.buildFirstInstructionPlan();
    if (equals(cmd, "hv first-instruction validate")) return hv.validateFirstInstructionPlan();
    if (equals(cmd, "hv first-instruction blockers")) return hv.blockersFirstInstructionPlan();
    if (equals(cmd, "hv first-instruction next")) return hv.nextFirstInstructionPlan();
    if (equals(cmd, "hv first-instruction chain")) return hv.chainFirstInstructionPlan();
    if (equals(cmd, "hv first-instruction checksum")) return hv.checksumFirstInstructionPlan();
    if (equals(cmd, "hv first-instruction reset")) return hv.resetFirstInstructionPlan();
    if (equals(cmd, "hv first-instruction invariant-consumption-test")) return hv.fipConsumption();
    if (equals(cmd, "hv first-instruction invariant-corruption-test")) return hv.fipCorruption();
    if (equals(cmd, "hv hv26 invariant-hv25-consumption-test")) return hv.hv26Hv25Consumption();
    if (equals(cmd, "hv hv26 invariant-all")) return hv.hv26All();
    if (equals(cmd, "hv first-instruction require-hgatp-test")) return hv.fipRequireHgatp();
    if (equals(cmd, "hv first-instruction require-activation-plan-test")) return hv.fipRequireActivation();
    if (equals(cmd, "hv first-instruction require-entry-frame-test")) return hv.fipRequireEntry();
    if (equals(cmd, "hv first-instruction require-trap-return-frame-test")) return hv.fipRequireTrap();
    if (equals(cmd, "hv first-instruction guest-entered-test")) return hv.fipGuest();
    if (equals(cmd, "hv first-instruction instruction-executed-test")) return hv.fipInstruction();
    if (equals(cmd, "hv first-instruction trap-return-executed-test")) return hv.fipTrap();
    if (equals(cmd, "hv first-instruction active-stage2-test")) return hv.fipStage2();
    if (equals(cmd, "hv first-instruction hgatp-written-test")) return hv.fipHgatpWritten();
