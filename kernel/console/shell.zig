...
fn handle(cmd: []const u8) void {
    ...
    if (equals(cmd, "hv hgatp-csr-boundary") or equals(cmd, "hv-hgatp-csr-boundary") or equals(cmd, "hv hgatp-csr-boundary status")) return hv.printCsrBoundary();
    if (equals(cmd, "hv hgatp-csr-boundary build")) return hv.createCsrBoundary();
    if (equals(cmd, "hv hgatp-csr-boundary validate")) return hv.validateCsrBoundary();
    if (equals(cmd, "hv hgatp-csr-boundary execute")) return hv.executeCsrBoundary();
    if (equals(cmd, "hv hgatp-csr-boundary fields")) return hv.inspectCsrBoundary();
    if (equals(cmd, "hv hgatp-csr-boundary request")) return hv.inspectCsrBoundary();
    if (equals(cmd, "hv hgatp-csr-boundary reset")) return hv.resetCsrBoundary();
    ...
    if (equals(cmd, "hv hgatp-execution-dry-run execute")) return hv.executeHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run blockers")) return hv.blockersHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run next")) return hv.nextHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run checksum")) return hv.checksumHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run reset")) return hv.resetHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run fields")) return hv.fieldsHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run request")) return hv.requestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run steps")) return hv.stepsHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run result")) return hv.resultHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run trap-slot")) return hv.trapSlotHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run readback")) return hv.readbackHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run decision")) return hv.decisionHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run require-operation-test")) return hv.requireOperationTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run invalid-operation-test")) return hv.invalidOperationTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run source-integrity-test")) return hv.sourceIntegrityTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run request-value-test")) return hv.requestValueTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run opt-in-test")) return hv.optInTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run policy-allows-test")) return hv.policyAllowsTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run operation-call-reachable-test")) return hv.operationCallReachableTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run operation-call-called-test")) return hv.operationCallCalledTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run raw-write-called-test")) return hv.rawWriteCalledTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run execution-reached-raw-write-test")) return hv.executionReachedRawWriteTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run execution-called-raw-write-test")) return hv.executionCalledRawWriteTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run fake-trap-test")) return hv.fakeTrapTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run fake-readback-test")) return hv.fakeReadbackTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run write-attempted-test")) return hv.writeAttemptedTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run write-performed-test")) return hv.writePerformedTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run active-stage2-test")) return hv.activeStage2TestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run guest-entered-test")) return hv.guestEnteredTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run first-instruction-test")) return hv.firstInstructionTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run invariant-consumption-test")) return hv.invariantConsumptionTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-execution-dry-run invariant-corruption-test")) return hv.invariantCorruptionTestHgatpExecutionDryRun();
    if (equals(cmd, "hv hgatp-hardware-write-operation") or equals(cmd, "hv-hgatp-hardware-write-operation") or equals(cmd, "hv hgatp-hardware-write-operation status")) return hv.printHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation build")) return hv.buildHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation validate")) return hv.validateHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation blockers")) return hv.blockersHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation next")) return hv.nextHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation checksum")) return hv.checksumHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation reset")) return hv.resetHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation fields")) return hv.fieldsHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation request")) return hv.requestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation preflight")) return hv.preflightHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation result")) return hv.resultHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation trap-slot")) return hv.trapSlotHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation readback")) return hv.readbackHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation decision")) return hv.decisionHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation require-prep-test")) return hv.requirePrepTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation invalid-prep-test")) return hv.invalidPrepTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation source-integrity-test")) return hv.sourceIntegrityTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation request-value-test")) return hv.requestValueTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation opt-in-test")) return hv.optInTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation policy-allows-test")) return hv.policyAllowsTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation call-reachable-test")) return hv.callReachableTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation call-called-test")) return hv.callCalledTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation raw-write-called-test")) return hv.rawWriteCalledTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation fake-trap-test")) return hv.fakeTrapTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation fake-readback-test")) return hv.fakeReadbackTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation write-attempted-test")) return hv.writeAttemptedTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation write-performed-test")) return hv.writePerformedTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation active-stage2-test")) return hv.activeStage2TestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation guest-entered-test")) return hv.guestEnteredTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation first-instruction-test")) return hv.firstInstructionTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation invariant-consumption-test")) return hv.invariantConsumptionTestHgatpHardwareWriteOperation();
    if (equals(cmd, "hv hgatp-hardware-write-operation invariant-corruption-test")) return hv.invariantCorruptionTestHgatpHardwareWriteOperation();
    ...
    if (equals(cmd, "hv hgatp-csr-result blockers")) return hv.blockersHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result next")) return hv.nextHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result checksum")) return hv.checksumHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result reset")) return hv.resetHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result fields")) return hv.fieldsHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result observation")) return hv.observationHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result trap-slot")) return hv.trapSlotHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result readback")) return hv.readbackHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result decision")) return hv.decisionHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result require-interface-test")) return hv.requireInterfaceTestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result invalid-interface-test")) return hv.invalidInterfaceTestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result source-integrity-test")) return hv.sourceIntegrityTestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result request-value-test")) return hv.requestValueTestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result csr-called-test")) return hv.csrCalledTestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result raw-asm-called-test")) return hv.rawAsmCalledTestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result write-attempted-test")) return hv.writeAttemptedTestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result write-performed-test")) return hv.writePerformedTestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result fake-fault-test")) return hv.fakeFaultTestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result fake-readback-test")) return hv.fakeReadbackTestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result active-stage2-test")) return hv.activeStage2TestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result guest-entered-test")) return hv.guestEnteredTestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result first-instruction-test")) return hv.firstInstructionTestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result invariant-consumption-test")) return hv.invariantConsumptionTestHgatpCsrResult();
    if (equals(cmd, "hv hgatp-csr-result invariant-corruption-test")) return hv.invariantCorruptionTestHgatpCsrResult();
    ...
    if (equals(cmd, "hv guest-entry-contract") or equals(cmd, "hv-guest-entry-contract") or equals(cmd, "hv guest-entry-contract status")) return hv.printGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract build")) return hv.buildGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract validate")) return hv.validateGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract fields")) return hv.fieldsGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract checksum")) return hv.checksumGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract decision")) return hv.decisionGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract blockers")) return hv.blockersGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract next")) return hv.nextGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract reset")) return hv.resetGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract source")) return hv.sourceGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract register-frame")) return hv.registerFrameGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract execution-frame")) return hv.executionFrameGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract trap-return-target")) return hv.trapReturnTargetGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract boot-sources")) return hv.bootSourcesGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract linux-handoff")) return hv.linuxHandoffGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract safety")) return hv.safetyGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract require-hv39-test")) return hv.requireHv39TestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract invalid-hv39-test")) return hv.invalidHv39TestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract source-integrity-test")) return hv.sourceIntegrityTestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract invalid-guest-pc-test")) return hv.invalidGuestPcTestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract invalid-guest-sp-test")) return hv.invalidGuestSpTestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract invalid-register-frame-test")) return hv.invalidRegisterFrameTestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract invalid-execution-frame-test")) return hv.invalidExecutionFrameTestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract invalid-trap-return-target-test")) return hv.invalidTrapReturnTargetTestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract guest-ready-test")) return hv.guestReadyTestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract trap-return-ready-test")) return hv.trapReturnReadyTestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract guest-entered-test")) return hv.guestEnteredTestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract first-instruction-test")) return hv.firstInstructionTestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract trap-return-executed-test")) return hv.trapReturnExecutedTestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract active-stage2-test")) return hv.activeStage2TestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract hgatp-written-test")) return hv.hgatpWrittenTestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract invariant-consumption-test")) return hv.invariantConsumptionTestGuestEntryContract();
    if (equals(cmd, "hv guest-entry-contract invariant-corruption-test")) return hv.invariantCorruptionTestGuestEntryContract();
    if (equals(cmd, "hv hgatp-csr-write-eligibility") or equals(cmd, "hv-hgatp-csr-write-eligibility") or equals(cmd, "hv hgatp-csr-write-eligibility status")) return hv.printHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility build")) return hv.buildHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility validate")) return hv.validateHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility evaluate")) return hv.evaluateHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility blockers")) return hv.blockersHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility next")) return hv.nextHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility checksum")) return hv.checksumHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility reset")) return hv.resetHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility fields")) return hv.fieldsHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility request")) return hv.requestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility steps")) return hv.stepsHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility result")) return hv.resultHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility trap-slot")) return hv.trapSlotHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility readback")) return hv.readbackHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility decision")) return hv.decisionHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility require-boundary-test")) return hv.requireBoundaryTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility invalid-boundary-test")) return hv.invalidBoundaryTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility source-integrity-test")) return hv.sourceIntegrityTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility request-value-test")) return hv.requestValueTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility policy-allows-test")) return hv.policyAllowsTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility csr-eligible-test")) return hv.csrEligibleTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility csr-reached-test")) return hv.csrReachedTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility csr-called-test")) return hv.csrCalledTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility raw-reached-test")) return hv.rawReachedTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility raw-called-test")) return hv.rawCalledTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility fake-trap-test")) return hv.fakeTrapTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility fake-fault-test")) return hv.fakeFaultTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility fake-readback-test")) return hv.fakeReadbackTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility write-attempted-test")) return hv.writeAttemptedTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility write-performed-test")) return hv.writePerformedTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility active-stage2-test")) return hv.activeStage2TestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility guest-entered-test")) return hv.guestEnteredTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility first-instruction-test")) return hv.firstInstructionTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility invariant-consumption-test")) return hv.invariantConsumptionTestHgatpCsrWriteEligibility();
    if (equals(cmd, "hv hgatp-csr-write-eligibility invariant-corruption-test")) return hv.invariantCorruptionTestHgatpCsrWriteEligibility();
