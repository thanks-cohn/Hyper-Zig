...
TRANSCRIPT="$ROOT/smoke/transcripts/latest-smoke-hv26-guest-entry-frame-v0.txt"; : > "$TRANSCRIPT"; ELF="$ROOT/zig-out/bin/zign01d-v0"
...
transcript=sys.argv[1]; cmd=sys.argv[2:]; commands=['hv guest-entry reset', 'hv guest-entry validate', 'hv guest-entry build', 'hv guest-entry registers', 'hv guest-entry pc-mutation-test', 'hv guest-entry sp-mutation-test', 'hv guest-entry invariant-mutation-test', 'hv guest-entry reset', 'shutdown']
...
for n in ['guest_entry_frame.state=empty', 'guest_entry_frame.validate_result=rejected', 'guest_entry_frame.build_result=ok', 'consumed_activation_plan_checksum=0x', 'consumed_context_checksum=0x', 'registers=structured', 'pc_mutation_test=ok', 'sp_mutation_test=ok', 'hv26.guest_entry.mutation_invariant=ok', 'guest_entered=false']:
...
print('PASS smoke-hv26-guest-entry-frame-v0.sh')
PYCHECK
