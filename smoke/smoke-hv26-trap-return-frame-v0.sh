...
TRANSCRIPT="$ROOT/smoke/transcripts/latest-smoke-hv26-trap-return-frame-v0.txt"; : > "$TRANSCRIPT"; ELF="$ROOT/zig-out/bin/zign01d-v0"
...
transcript=sys.argv[1]; cmd=sys.argv[2:]; commands=['hv trap-return reset', 'hv trap-return validate', 'hv trap-return build', 'hv trap-return invariant-mutation-test', 'hv trap-return sepc-corruption-test', 'hv trap-return status-corruption-test', 'hv trap-return reset', 'shutdown']
...
for n in ['trap_return_frame.state=empty', 'trap_return_frame.validate_result=rejected', 'trap_return_frame.build_result=ok', 'sepc_candidate=0x', 'sstatus_candidate=0x', 'entry_frame_checksum=0x', 'csr_safety_checksum=0x', 'hv26.trap_return.mutation_invariant=ok', 'sepc-corrupt', 'status-corrupt']:
...
print('PASS smoke-hv26-trap-return-frame-v0.sh')
PYCHECK
