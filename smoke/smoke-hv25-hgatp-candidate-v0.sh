...
LOG_DIR="$ROOT/logs/latest"; TRANSCRIPT="$ROOT/smoke/transcripts/latest-hv25-hgatp-candidate-v0.txt"; ELF="$ROOT/zig-out/bin/zign01d-v0"
...
COMMANDS=("hv hgatp" "hv hgatp validate" "hv hgatp build" "hv hgatp validate" "hv hgatp checksum" "hv hgatp fields" "hv hgatp reset" "hv hgatp validate" "shutdown")
...
tr=sys.argv[1]; cmd=sys.argv[2:]; cmds=["hv hgatp","hv hgatp validate","hv hgatp build","hv hgatp validate","hv hgatp checksum","hv hgatp fields","hv hgatp reset","hv hgatp validate","shutdown"]
...
import re,sys
text=open(sys.argv[1],errors='replace').read(); cmds=["hv hgatp","hv hgatp validate","hv hgatp build","hv hgatp validate","hv hgatp checksum","hv hgatp fields","hv hgatp reset","hv hgatp validate"]
blocks=[]; pos=0
for c in cmds:
...
need=[(0,'hv: hgatp_candidate.state=empty'),(1,'hv: hgatp_candidate.validate_result=rejected'),(1,'hv: hgatp_candidate.blocker=empty-candidate'),(2,'hv: hgatp_candidate.build_result=ok'),(2,'hv: hgatp_candidate.state=validated'),(3,'hv: hgatp_candidate.validate_result=ok'),(4,'hv: hgatp_candidate.checksum=0x'),(5,'hv: hgatp_candidate.mode='),(5,'hv: hgatp_candidate.vmid='),(5,'hv: hgatp_candidate.root_ppn='),(5,'hv: hgatp_candidate.candidate_value='),(6,'hv: hgatp_candidate.reset_result=ok'),(6,'hv: hgatp_candidate.state=empty'),(7,'hv: hgatp_candidate.validate_result=rejected')]
for i,m in need: assert m in blocks[i], f'missing {m}'
assert 'hv: hgatp_candidate.checksum=0x0' not in blocks[4], 'zero checksum after build'
