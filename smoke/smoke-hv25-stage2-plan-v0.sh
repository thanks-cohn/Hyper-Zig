...
TRANSCRIPT="$ROOT/smoke/transcripts/latest-smoke-hv25-stage2-plan-v0.txt"; : > "$TRANSCRIPT"; ELF="$ROOT/zig-out/bin/zign01d-v0"
...
transcript=sys.argv[1]; cmd=sys.argv[2:]; commands=['hv stage2-plan reset', 'hv stage2-plan build', 'hv stage2-plan validate', 'hv stage2-plan next', 'hv stage2-plan invariant-lifecycle-test', 'hv stage2-plan invariant-consumption-test']+["shutdown"]
...
text=Path(sys.argv[1]).read_text(errors='replace'); cmds=['hv stage2-plan reset', 'hv stage2-plan build', 'hv stage2-plan validate', 'hv stage2-plan next', 'hv stage2-plan invariant-lifecycle-test', 'hv stage2-plan invariant-consumption-test']; blocks={}; cur=0
for c in cmds:
...
for c,needles in {'hv stage2-plan reset': ['hv: stage2_plan.state=empty'], 'hv stage2-plan build': ['hv: stage2_plan.build_result=ok', 'hv: stage2_plan.checksum=0x'], 'hv stage2-plan validate': ['hv: stage2_plan.validate_result=ok', 'hv: stage2_plan.ready=true'], 'hv stage2-plan next': ['hv: stage2_plan.next=wait_for_safe_hgatp_write_guard'], 'hv stage2-plan invariant-lifecycle-test': ['hv: hv25.stage2_plan.lifecycle_invariant=ok'], 'hv stage2-plan invariant-consumption-test': ['hv: hv25.stage2_plan.consumption_invariant=ok']}.items():
 for n in needles:
...
print('PASS smoke-hv25-stage2-plan-v0.sh')
PYCHECK
