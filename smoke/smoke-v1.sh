...
log() { printf '[%s][ZIGN01D][INFO][SMOKE][SMOKE101] %s\n' "$(stamp)" "$*" | tee -a "$SMOKE_LOG"; }
fail_note() {
  {
    ...
    echo "  likely cause: boot, shell command dispatch, or honest placeholder marker missing"
...
set +e
{
  ...
  printf 'uptime\n'
  printf 'shutdown\n'
} | timeout 20s "${QEMU_CMD[@]}" >"$TRANSCRIPT" 2>&1
QEMU_STATUS=$?
set -e
