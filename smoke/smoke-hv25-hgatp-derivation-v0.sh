...
TRANSCRIPT="$ROOT/smoke/transcripts/latest-smoke-hv25-hgatp-derivation-v0.txt"; : > "$TRANSCRIPT"; ELF="$ROOT/zig-out/bin/zign01d-v0"
...
transcript=sys.argv[1]; cmd=sys.argv[2:]; commands=['hv hgatp invariant-derivation-test', 'hv hgatp invariant-corruption-test', 'hv hgatp candidate', 'hv hgatp checksum']+["shutdown"]
...
text=Path(sys.argv[1]).read_text(errors='replace'); cmds=['hv hgatp invariant-derivation-test', 'hv hgatp invariant-corruption-test', 'hv hgatp candidate', 'hv hgatp checksum']; blocks={}; cur=0
for c in cmds:
...
for c,needles in {'hv hgatp invariant-derivation-test': ['hv: hv25.hgatp.derivation_invariant=ok'], 'hv hgatp invariant-corruption-test': ['hv: hv25.hgatp.corruption_invariant=ok', 'hv: hgatp.last_error=invalid-vmid'], 'hv hgatp candidate': ['hv: hgatp.candidate=0x'], 'hv hgatp checksum': ['hv: hgatp.checksum=0x']}.items():
 for n in needles:
...
print('PASS smoke-hv25-hgatp-derivation-v0.sh')
PYCHECK
