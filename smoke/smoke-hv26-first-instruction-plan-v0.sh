...
TRANSCRIPT="$ROOT/smoke/transcripts/latest-smoke-hv26-first-instruction-plan-v0.txt"; : > "$TRANSCRIPT"; ELF="$ROOT/zig-out/bin/zign01d-v0"
...
transcript=sys.argv[1]; cmd=sys.argv[2:]; commands=['hv first-instruction reset', 'hv first-instruction validate', 'hv first-instruction build', 'hv first-instruction chain', 'hv first-instruction next', 'hv first-instruction invariant-consumption-test', 'hv first-instruction reset', 'shutdown']
...
for n in ['first_instruction_plan.state=empty', 'first_instruction_plan.validate_result=rejected', 'first_instruction_plan.build_result=ok', 'execution_chain_checksum=0x', 'consumed_hgatp_checksum=0x', 'consumed_activation_plan_checksum=0x', 'dependency=hgatp-candidate', 'next_required_action=guarded_trap_return_assembly_preparation', 'hv26.first_instruction.consumption_invariant=ok']:
...
print('PASS smoke-hv26-first-instruction-plan-v0.sh')
PYCHECK
