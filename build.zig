...
const hyperzig_status_text =
    ...
    \\  next milestone: HV14 DTB/SBI/active guest-entry prerequisites (no Linux claim)
    ...
    \\  VM/vCPU model, vCPU lifecycle, guest-memory object, guest-address-space metadata, tiny-flat-v0 guest-image loader, guest-entry preparation, guest trap/exit metadata, controlled guest-run attempt safety gate, hardware-gated execution preparation, and second-stage translation metadata, the software-only stage2 table builder, and the guest boot package contract implemented and smoke-proven when validation passes
