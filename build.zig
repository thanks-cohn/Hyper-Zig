...
const hyperzig_status_text =
    ...
    \\  Zig target: 0.14.x
    \\  current proven milestones: HV0 through HV14 when smoke validation passes
    \\  next milestone: HV15 SBI foundation or controlled active guest-entry prerequisites (no Linux claim)
    ...
    \\  VM/vCPU model, vCPU lifecycle, guest-memory object, guest-address-space metadata, tiny-flat-v0 guest-image loader, guest-entry preparation, guest trap/exit metadata, controlled guest-run attempt safety gate, hardware-gated execution preparation, and second-stage translation metadata, the software-only stage2 table builder, and the guest boot package contract and guest DTB contract implemented and smoke-proven when validation passes
