...
const hyperzig_status_text =
    ...
    \\  Zig target: 0.14.x
    \\  current proven milestones: HV0, HV1, HV2, HV3, HV4, HV5, HV6, HV7, HV8, HV9 when smoke validation passes
    \\  next milestone: HV10 first hardware-gated guest execution research (no Linux claim)
    \\  canonical validation command: ./scripts/validate-hyperzig.sh
    \\  no Linux guest support yet
    \\  no guest execution yet
    \\  no guest execution yet; HV7 prepares guest-entry metadata, HV8 records simulated exit metadata, and HV9 arms no-execute run-attempt metadata only
    \\  no second-stage translation yet
    \\  VM/vCPU model, vCPU lifecycle, guest-memory object, guest-address-space metadata, tiny-flat-v0 guest-image loader, guest-entry preparation, and guest trap/exit metadata, and controlled guest-run attempt safety gate implemented and smoke-proven when validation passes
