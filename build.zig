...
const hyperzig_status_text =
    ...
    \\  Zig target: 0.14.x
    \\  current proven milestones: HV0, HV1, HV2, HV3, HV4, HV5, HV6, HV7, HV8 when smoke validation passes
    ...
    \\  no guest execution yet; HV7 only prepares guest-entry metadata and HV8 records simulated exit metadata
    \\  no second-stage translation yet
    \\  VM/vCPU model, vCPU lifecycle, guest-memory object, guest-address-space metadata, tiny-flat-v0 guest-image loader, guest-entry preparation, and guest trap/exit metadata implemented and smoke-proven when validation passes
